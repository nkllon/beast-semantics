...
export const summarizeCategories = async (operations) => {
...
export const determineCategoryPolicies = async (summaries, resolved, io) => {
    const policies = {};
    ...
    const applyDefault = (value) => {
        if (commands)
            policies.commands = value;
        if (settings)
            policies.settings = value;
        if (projectMemory)
            policies['project-memory'] = value === 'overwrite' ? 'overwrite' : 'skip';
    };
    ...
    if (!interactive) {
        io.log(formatWarning('Non-interactive environment detected. Existing files will be kept. Use --yes or --overwrite=force to overwrite.'));
        return policies;
    }
    // Commands (prompt once)
    if (commands && hasConflicts(commands)) {
        const confirm = await promptConfirm(`Update commands in ${categoryDescriptions('commands', resolved)}? (recommended: Yes)`, true);
        policies.commands = confirm ? 'overwrite' : 'skip';
    }
    // Settings templates/rules
    if (settings && hasConflicts(settings)) {
        const confirm = await promptConfirm(`Update settings templates and rules in ${categoryDescriptions('settings', resolved)}?`, true);
    ...
    if (projectMemory && hasConflicts(projectMemory)) {
        ...
        const choice = await promptChoice(`Project Memory document (${docName}) already exists. How should we apply updates? (recommended: Overwrite)`, [
            {
                value: 'overwrite',
