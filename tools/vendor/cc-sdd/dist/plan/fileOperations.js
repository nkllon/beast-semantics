...
const walkDir = async (dir) => {
    const out = [];
    const queue = [{ path: dir, isDirectory: true }];
    while (queue.length) {
...
const transformTemplateOutput = (relPath) => {
...
const determineModeFromFilename = (filename) => {
    if (filename.endsWith('.json'))
        return 'json';
    return 'text';
};
export const buildFileOperations = async (artifacts, resolved, opts = {}) => {
